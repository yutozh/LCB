...
function Account(scope, cb) {
    ...
    },
    {
      ...
      conv: Array,
      expression: "(select GROUP_CONCAT(dependentId) from " + this.table + "2lcs where accountId = a.address)"
    },
    {
      name: "u_lcs",
      type: "Text",
      ...
      conv: Array,
      expression: "(select GROUP_CONCAT(dependentId) from " + this.table + "2u_lcs where accountId = a.address)"
    },
    {
      name: "auth",
      type: "Text",
      ...
      conv: Array,
      expression: "(select GROUP_CONCAT(dependentId) from " + this.table + "2auth where accountId = a.address)"
    },
    {
      name: "u_auth",
      type: "Text",
      ...
      conv: Array,
      expression: "(select GROUP_CONCAT(dependentId) from " + this.table + "2u_auth where accountId = a.address)"
    },
	];
...
Account.prototype.createTables = function (cb) {
  ...
  sqles.push(sql.query);
	...
	sqles.push("delete from mem_accounts2u_lcs;");
  sqles.push("delete from mem_accounts2u_auth;");
	...
	sqles.push("INSERT INTO mem_accounts2u_lcs SELECT * FROM mem_accounts2lcs;");
  sqles.push("INSERT INTO mem_accounts2u_auth SELECT * FROM mem_accounts2auth;");
		...
		if (diff[value]) {
			...
			switch (self.conv[value]) {
				...
				case Array:
					if (Object.prototype.toString.call(trueValue[0]) == "[object Object]") {
						...
						for (var i = 0; i < trueValue.length; i++) {
							...
							if (math == "-") {
								...
								if (value == "delegates") {
									round.push({
										query: "insert into mem_round (address, amount, delegate, blockId, round) select $address, -1, $delegate, $blockId, $round from mem_accounts where address = $address",
										values: {
											address: address,
											delegate: val,
											blockId: diff.blockId,
                  ...
                  round.push({
                    query: "insert into mem_round_lc (address, amount, lc, blockId, round) values($address, -1, $lc, $blockId, $round)",
                    values: {
                      address: address,
                      lc: val,
                      blockId: diff.blockId,
