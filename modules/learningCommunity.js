...
function LC() {
  ...
  this.verify = function (trs, sender, cb) {
    ...
    modules.accounts.getAccount({
      username: trs.asset.lc.username
  ...
  this.getBytes = function (trs) {
    ...
    try {
      var buf = new Buffer(trs.asset.lc.username, 'utf8');
    } catch (e) {
  ...
  this.apply = function (trs, block, sender, cb) {
    ...
    if (!sender.nameexist && trs.asset.lc.username) {
      data.u_username = null;
      data.username = trs.asset.lc.username;
    }
  ...
  this.undo = function (trs, block, sender, cb) {
    ...
    if (!sender.nameexist && trs.asset.lc.username) {
      data.username = null;
      data.u_username = trs.asset.lc.username;
    }
  ...
  this.applyUnconfirmed = function (trs, sender, cb) {
    if (sender.u_username && trs.asset.lc.username && trs.asset.lc.username != sender.u_username) {
      return cb("Account already has a username");
    }
    ...
    modules.accounts.getAccount({
      u_username: trs.asset.lc.username
    }, function (err, account) {
      if (err) {
        return cb(err);
      }
  ...
  this.objectNormalize = function (trs) {
    var report = library.scheme.validate(trs.asset.lc, {
      type: "object",
      properties: {
        publicKey: {
          type: "string",
    ...
    if (!report) {
      throw Error("Can't verify lc transaction, incorrect parameters: " + library.scheme.getLastError());
    }
  ...
  this.dbSave = function (trs, cb) {
    library.dbLite.query("INSERT INTO lcs(username, transactionId) VALUES($username, $transactionId)", {
      username: trs.asset.lc.username,
...
// Constructor
function LCs(cb, scope) {
  library = scope;
  self = this;
  self.__private = privated;
  privated.attachApi();

  library.logic.transaction.attachAssetType(TransactionTypes.LC, new LC());
...
privated.attachApi = function () {
  ...
  library.network.app.use('/api/lcs', router);
...
privated.getKeysSortByVote = function (cb) {
  modules.accounts.getAccounts({
    ...
    lc_vote:{$gt: constants.delegates * 2.0 / 3}
...
// Public methods
LCs.prototype.generateDelegateList = function (height, cb) {
...
LCs.prototype.checkLCs = function (publicKey, votes, cb) {
  if (util.isArray(votes)) {
    modules.accounts.getAccount({publicKey: publicKey}, function (err, account) {
      ...
      // !!
      if (!account.isDelegate){
        return cb("Only Delegates Account can vote to lc");
      }
      ...
      async.eachSeries(votes, function (action, cb) {
        ...
        if (math == "+" && (account.lcs !== null && account.lcs.indexOf(publicKey) != -1)) {
          return cb("Failed to add vote, account has already voted for this lc");
        }
        if (math == "-" && (account.lcs === null || account.lcs.indexOf(publicKey) === -1)) {
          return cb("Failed to remove vote, account has not voted for this lc");
        }

        modules.accounts.getAccount({publicKey: publicKey, isLC: 1}, function (err, account) {
          if (err) {
            return cb(err);
          }

          if (!account) {
            return cb("LC not found");
          }
...
// 检查授权地址，要求是LC
LCs.prototype.checkAuth = function (publicKey, votes, cb) {
  if (util.isArray(votes)) {
    modules.accounts.getAccount({publicKey: publicKey}, function (err, account) {
      if (err) {
        return cb(err);
      ...
      async.eachSeries(votes, function (action, cb) {
        ...
        if (math == "+" && (account.auth !== null && account.auth.indexOf(publicKey) != -1)) {
          return cb("Failed to auth, account has already auth for this lc");
        }
        if (math == "-" && (account.auth === null || account.auth.indexOf(publicKey) === -1)) {
          return cb("Failed to remove auth, account has not auth for this lc");
        }
    ...
    setImmediate(cb, "Please provide an array of auth");
  }
};


LCs.prototype.checkUnconfirmedLCs = function (publicKey, votes, cb) {
  ...
  if (util.isArray(votes)) {
    modules.accounts.getAccount({publicKey: publicKey}, function (err, account) {
      ...
      async.eachSeries(votes, function (action, cb) {
        ...
        if (math == "+" && (account.u_lcs !== null && account.u_lcs.indexOf(publicKey) != -1)) {
          return cb("Failed to add vote, account has already voted for this lc");
        }
...
LCs.prototype.checkUnconfirmedAuth = function (publicKey, votes, cb) {
  ...
  if (util.isArray(votes)) {
    modules.accounts.getAccount({publicKey: publicKey}, function (err, account) {
      ...
      async.eachSeries(votes, function (action, cb) {
        ...
        if (math == "+" && (account.u_auth !== null && account.u_auth.indexOf(publicKey) != -1)) {
          return cb("Failed to add auth, account has already auth for this lc");
        }
    ...
    return setImmediate(cb, "Please provide an array of auth");
  }
};

LCs.prototype.fork = function (block, cause) {
...
LCs.prototype.validateBlockSlot = function (block, cb) {
...
LCs.prototype.sandboxApi = function (call, args, cb) {
  sandboxHelper.callMethod(shared, call, args, cb);
};

// Events
LCs.prototype.onBind = function (scope) {
  modules = scope;
};

LCs.prototype.onBlockchainReady = function () {
...
LCs.prototype.cleanup = function (cb) {
  privated.loaded = false;
  cb();
};

// Shared
shared.getLC = function (req, cb) {
  var query = req.body;
  library.scheme.validate(query, {
    type: "object",
    properties: {
    ...
    modules.accounts.getAccounts({
      isLC: 1,
      sort: {"lc_vote": -1, "publicKey": 1}
    }, ["username", "address", "publicKey", "lc_vote", "virgin"], function (err, lcs) {
      if (err) {
      ...
      var lc = lcs.find(function (lc) {
        if (query.transactionId) {
          // TODO: Store transactionId
        }
        if (query.publicKey) {
          return lc.publicKey == query.publicKey;
        }
        if (query.username) {
          return lc.username == query.username;
        }
...
shared.getLCVoters = function (req, cb) {
  var query = req.body;
  library.scheme.validate(query, {
    type: 'object',
    properties: {
    ...
    library.dbLite.query("select GROUP_CONCAT(accountId) from mem_accounts2lcs where dependentId = $publicKey", {
...
shared.getLCs = function (req, cb) {
  ...
  library.scheme.validate(query, {
    ...
    modules.accounts.getAccounts({
      ...
      var result = lcs.slice(offset, realLimit);

      cb(null, {lcs: result, totalCount: count});
    });
  });
};
