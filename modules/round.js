...
Round.prototype.getLC_Votes = function (round, cb) {
  library.dbLite.query("select lc, amount from ( " +
    "select m.lc, sum(m.amount) amount, m.round from mem_round_lc m " +
    "group by m.lc, m.round " +
    ") where round = $round", {round: round}, {lc: String, amount: Number}, function (err, rows) {
    cb(err, rows);
  });
};
...
Round.prototype.flush_lc = function (round, cb) {
	library.dbLite.query("delete from mem_round_lc where round = $round", {round: round}, cb);
};
          ...
          function (cb) {
            modules.accounts.getAccounts({
              ...
              for (var i = 0; i < delegates.length; i++) {
              	for(var j=0; j<delegates[i].lcs.length; j++){
                  if(voteObj[delegates[i].lcs[j]] === undefined){
                    voteObj[delegates[i].lcs[j]] = 1
									}else {
                    voteObj[delegates[i].lcs[j]] += 1
									}
								}
              ...
              let voteList = []
              	...
              	let item = {}
              	item.lc = key
								item.amount = voteObj[key]
								voteList.push(item)
							}

              async.eachSeries(voteList, function (vote, cb) {
                library.dbLite.query('update mem_accounts set lc_vote = $amount where address = $address', {
                  address: modules.accounts.generateAddressByPublicKey(vote.lc),
                  amount: vote.amount
                }, cb);
              }, function (err) {
                self.flush_lc(round, function (err2) {
                  cb(err || err2);
                });
              });
            });
            // self.getLC_Votes(round, function (err, votes) {
            //   if (err) {
            //     return cb(err);
            //   }
            //   async.eachSeries(votes, function (vote, cb) {
            //     library.dbLite.query('update mem_accounts set lc_vote = lc_vote + $amount where address = $address', {
            //       address: modules.accounts.generateAddressByPublicKey(vote.lc),
            //       amount: vote.amount
            //     }, cb);
            //   }, function (err) {
