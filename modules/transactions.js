			...
			modules.accounts.getAccount(query, function (err, recipient) {
				...
				if (body.multisigAccountPublicKey && body.multisigAccountPublicKey != keypair.publicKey.toString('hex')) {
					...
					modules.accounts.getAccount({publicKey: keypair.publicKey.toString('hex')}, function (err, account) {
						...
						// MY LC
						if (!account.isLC){
							return cb("Only LC can change users' authority value")
						}

            // 查看接受方是否授权给发送方（接收方auth字段里是否有发送方）
            if (recipient.auth.indexOf(keypair.publicKey.toString('hex')) < 0){
              return cb("Recipient not auth to you");
            }
						// if (account.lc_vote < 2 * constants.delegates / 3){
             //  return cb("Although you are a LC , but your lc_vote is less than 2/3, " +
						// 		"you can not change users' authority value now. | Your current lc_vote is " + account.lc_vote)
						// }
            if (account.lc_vote < 1){
              return cb("Although you are a LC , but your lc_vote is less than 2/3, " +
                "you can not change users' authority value now. | Your current lc_vote is " + account.lc_vote)
            }
