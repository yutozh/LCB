...
describe('Transactions', function() {
    ...
    before(function (done) {
        // Send to EBOOKCOIN to account 1 address
        setTimeout(function() {
            randomEbookcoin = node.randomizeEbookcoin();
            expectedFee = node.expectedFee(randomEbookcoin);
            node.api.put('/transactions')
                .set('Accept', 'application/json')
                .send({
                    secret: node.Faccount.password,
                    amount: randomEbookcoin,
                ...
                .end(function (err, res) {
                    ...
                    if (res.body.success == true && res.body.transactionId != null) {
                        console.log('Sent to ' + Account1.address + ' ' + (randomEbookcoin / node.normalizer) + ' EBOOKCOIN');
                        console.log('Expected fee (paid by sender): ' + expectedFee / node.normalizer + ' EBOOKCOIN');
                        Account1.transactions.push(transactionCount);
                        transactionCount += 1;
                        totalTxFee += (expectedFee / node.normalizer);
                        Account1.balance += randomEbookcoin;
                        transactionList[transactionCount - 1] = {
                            'sender': node.Faccount.address,
                            'recipient': Account1.address,
                            'brutoSent': (randomEbookcoin + expectedFee) / node.normalizer,
                            'fee': expectedFee / node.normalizer,
                            'nettoSent': randomEbookcoin / node.normalizer,
                            'txId': res.body.transactionId,
                            'type':node.TxTypes.SEND
                        }
                    }
                    else{
                        console.log("Sending EBOOKCOIN to Account1 failed.");
                        console.log("Sent: secret: " + node.Faccount.password + ", amount: " + randomEbookcoin + ", recipientId: " + Account1.address );
                        node.expect("TEST").to.equal("FAILED");
                    }
                    done();
                });
    ...
    before(function (done) {
        ...
        setTimeout(function() {
            ...
            node.api.put('/transactions')
                ...
                .end(function (err, res) {
                    console.log(JSON.stringify(res.body));
                    console.log('We send the EBOOKCOIN from foundation account to account. Recipient is: ' + Account2.address);
                    console.log('Sent to ' + Account2.address + ' ' + (randomEbookcoin / node.normalizer) + ' EBOOKCOIN');
                    console.log('Expected fee (paid by sender): ' + expectedFee / node.normalizer + ' EBOOKCOIN');
                    node.expect(res.body).to.have.property("success").to.be.true;
                    if (res.body.success == true && res.body.transactionId != null) {
                        ...
                        transactionCount += 1;
                        totalTxFee += (expectedFee / node.normalizer);
                        Account2.balance += randomEbookcoin;
                        transactionList[transactionCount - 1] = {
                            'sender': node.Faccount.address,
                        ...
                        console.log("Sending EBOOKCOIN to Account2 failed.");
                        console.log("Sent: secret: " + node.Faccount.password + ", amount: " + randomEbookcoin + ", recipientId: " + Account2.address );
                        node.expect("TEST").to.equal("FAILED");
                    }
                    done();
                });
      ...
      node.expect(err).to.be.not.ok;
        ...
        describe('/transactions', function () {
            ...
            it(test + '. Attempting to get transactions list. Sending INVALID FIELDS. Expecting error', function (done) {
                var senderId = "notAReadAddress", blockId = 'about5', recipientId = 'EbookcoinLIOnair3', limit = 'aLOT', offset = 'Boris', orderBy = 't_blockId:asc';
          ...
          node.expect(err).to.be.not.ok;
            ...
            test += 1;
            it(test + '. We try to send NEGATIVE EBOOKCOIN VALUE from Account 1 to Account 2. We expect error',function(done){
                    amountToSend = -100000000;
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
                        .send({
            ...
            test += 1;
            it(test + '. We try to send INVALID EBOOKCOIN VALUE from Account 1 to Account 2. We expect error',function(done){
                amountToSend = 1.2;
                node.api.put('/transactions')
                    .set('Accept', 'application/json')
                    .send({
            ...
            test += 1;
            it(test + '. We try to send 100% EBOOKCOIN as amout from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send 0 EBOOKCOIN from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY LARGE EBOOKCOIN NUMBER from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY LARGE NEGATIVE EBOOKCOIN NUMBER of EBOOKCOIN from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY SMALL EBOOKCOIN NUMBER (0.00000001) from Account 1 to Account 2. We get success',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY SMALL NEGATIVE EBOOKCOIN NUMBER (0.00000001) from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We attempt to created 2nd password from account with 0 EBOOKCOIN. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
