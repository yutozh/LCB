...
console.log("Random EBOOKCOIN is: " + (node.EBOOKCOIN / node.normalizer));
...
describe('Delegates', function() {
    ...
    describe('Vote and Register delegate attempts from account with 0 EBOOKCOIN', function() {

        test += 1;
        it(test + '. We attempt to upVote a delegate from the new random account. We expect error (account should have 0 EBOOKCOIN)',function(done){
        ...
        test += 1;
        it(test + '. We attempt to downVote a delegate from the new random account. We expect error (account should have 0 EBOOKCOIN)',function(done){
            node.onNewBlock(function(err) {
                node.api.put('/accounts/delegates')
                    .set('Accept', 'application/json')
                    .send({
        ...
        test += 1;
        it(test + '. We attempt to register as delegate from random account. Since we have 0 EBOOKCOIN, we expect error',function(done){
    ...
    describe('upVoting and downVoting',function() {
            ...
            node.api.put('/transactions')
                ...
                .end(function (err, res) {
                    ...
                    if (res.body.success == true && res.body.transactionId != null){
                        ...
                        Raccount.amount += node.EBOOKCOIN;
                    }
                    else{
                        console.log("Transaction failed or transactionId is null");
                        console.log("Sent: secret: " + node.Faccount.password + ", amount: " + node.EBOOKCOIN + ", recipientId: " + Raccount.address);
                        node.expect("TEST").to.equal("FAILED");
                    }
                    done();
                });
        ...
        before(function (done) {
            // Check that Raccount has the EBOOKCOIN we sent
                ...
                node.api.post('/accounts/open')
                    ...
                    .end(function (err, res) {
                        ...
                        if (res.body.success == true && res.body.account != null){
                            node.expect(res.body.account.balance).to.be.equal(node.EBOOKCOIN);
                        }
                        else{
    ...
    describe('Get Delegates list',function() {
        ...
        it(test + '. We attempt to get delegate via address parameter but we send INVALID ADDRESS. We expect error',function(done){
            node.api.get('/accounts/delegates?address=NOTaEbookcoinAddress')
                .set('Accept', 'application/json')
                .expect('Content-Type', /json/)
                .expect(200)
                .end(function (err, res) {
